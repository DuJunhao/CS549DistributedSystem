...
public class State implements IState, IRouting {
	...
	public synchronized void add(String k, String v) {
		...
		vl.add(v);
		// TOD: broadcast an event to any listeners. 
	...
	public synchronized NodeInfo closestPrecedingFinger(int id) {
		/*
		 * TOD: Get closest preceding finger for id, to continue search at that
	...
	public void removeListener(int id, String key) {
		// TOD Close the event output stream.
		if(listeners.containsKey(key)){
	...
	private void broadcastAddition(String key, String value) {
		// TOD broadcast an added binding (use IDHTNode.NEW_BINDING_EVENT for event name).	
		if(listeners.containsKey(key)) {
	...
	public void removeCallback(String key) {
		// TOD remove an existing callback (if any) for bindings on key.
