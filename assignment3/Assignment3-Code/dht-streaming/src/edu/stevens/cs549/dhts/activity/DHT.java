...
//		routing.joinCheck();
	...
	protected NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws Failed {
		...
		if (localInfo.equals(info)) {
			...
			if (IRouting.USE_FINGER_TABLE) {
				/*
				 * TOD: Do the Web service call to the remote node.
				 */
	...
	public void join(String uri) throws Failed, Invalid {
		...
		NodeInfo succ;
		/*
		 * TOD: Do a web service call to the node identified by "uri" and find
	...
	public EventOutput listenForBindings(int id, String key) {
		// TOD create event output stream and add to broadcaster.
	...
	public void stopListening(int id, String key) {
		// TOD remove event output stream from broadcaster.
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		/*
		 * TOD: Register a listener for new bindings under key, at the node
	...
	public void listenOff(String key) throws DHTBase.Failed {
		/*
		 * TOD: Stop listening for new binding events for this key.  Need to
